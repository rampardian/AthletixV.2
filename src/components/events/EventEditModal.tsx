...
const formSchema = z.object({
  ...
  type: z.enum(["Competition","Tryout","Camp","Showcase","Clinic","Tournament","Seminar","Other"]),
...
export default function EventEditModal({ open, onClose, event, onEventUpdated }: EventEditModalProps) {
  ...
  const { data: categories } = useFetch("http://localhost:5000/api/edit-event/categories");
  const { data: sponsors } = useFetch("http://localhost:5000/api/edit-event/sponsors");
      ...
      fetch(`http://localhost:5000/api/edit-event/${event.event_id}/details`)
        ...
        .then(data => {
          ...
          form.reset({
            ...
            endTime: format(new Date(data.end_datetime), "HH:mm"),
  ...
  const toggleSelection = (field: any, id: string) => {
    ...
    field.onChange(values.includes(id) ? values.filter(v => v !== id) : [...values, id]);
  };
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const existingCategoryIds = values.category_ids?.filter((v) =>
        categories.some((c: any) => c.category_id === v)
      ) || [];

      const newCategories = values.category_ids?.filter(
        (v) => !categories.some((c: any) => c.category_id === v)
      ) || [];

      const existingSponsorIds = values.sponsor_ids?.filter((v) =>
        sponsors.some((s: any) => s.sponsor_id === v)
      ) || [];

      const newSponsors = values.sponsor_ids?.filter(
        (v) => !sponsors.some((s: any) => s.sponsor_id === v)
      ) || [];

      const res = await fetch(`http://localhost:5000/api/edit-event/${event.event_id}`, {
        method: "PUT",
  ...
  const renderBadgeInput = (field: any, items: any[], idKey: string = "category_id", nameKey: string = "name") => {
    ...
    return (
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          ...
          {(field.value || []).filter(v => !items.some(i => i[idKey] === v)).map(v => (
            <Badge
              key={v}
              variant="default"
              className="cursor-pointer"
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="type" render={({ field }) => (
                <FormItem>
                  ...
                  {renderSelectInput(field, ["Competition","Tryout","Camp","Showcase","Clinic","Tournament","Seminar","Other"])}
                  <FormMessage />
                </FormItem>
              )} />
            </div>
            ...
            <FormField control={form.control} name="sport" render={({ field }) => (
              <FormItem>
                ...
                {sportsLoading ? <p>Loading...</p> : sportsError ? <p className="text-red-500">Failed to load</p> :
                  renderSelectInput(field, sports.map(s => s.sport_name))
                }
                <FormMessage />
              </FormItem>
            )} />
              ...
              <FormField control={form.control} name="endDate" render={({ field }) => (
                <FormItem>
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0">
                      <Calendar mode="single" selected={field.value} onSelect={field.onChange} disabled={d => d < new Date(form.getValues("startDate"))} initialFocus />
                    </PopoverContent>
                  </Popover>
                  <FormMessage />
                </FormItem>
            ...
            <FormField control={form.control} name="sponsor_ids" render={({ field }) => (
              <FormItem>
